...
class TestUserEdit(BaseCase):
    def test_edit_just_created_user(self):
        ...
        response = requests.post("https://playground.learnqa.ru/api/user/", data=register_data, verify=False)
        ...
        response = requests.post('https://playground.learnqa.ru/api/user/login', data=login_data, verify=False)
        Assertions.assert_status_code(response, 200)
        ...
        # GET USER DATA
        response = requests.get(f"https://playground.learnqa.ru/api/user/{user_id}",
                                headers={'x-csrf-token': token},
                                cookies={'auth_sid': auth_sid},
                                verify=False)
    ...
    def test_edit_from_another_user(self):
        # REGISTER FIRST USER
        register_data = self.prepare_registration_data()
        response1 = requests.post("https://playground.learnqa.ru/api/user/", data=register_data, verify=False)
        ...
        register_data = self.prepare_registration_data()
        response2 = requests.post("https://playground.learnqa.ru/api/user/", data=register_data, verify=False)
        ...
        response3 = requests.post('https://playground.learnqa.ru/api/user/login', data=login_data, verify=False)
        ...
        # EDIT SECOND USER
        new_name = 'Changed_name'
        response4 = requests.put(f"https://playground.learnqa.ru/api/user/{user_2_id}",
        ...
        # GET USER DATA
        response5 = requests.get(f"https://playground.learnqa.ru/api/user/{user_2_id}",
                                 headers={'x-csrf-token': token},
                                 cookies={'auth_sid': auth_sid},
                                 verify=False)
